...
class EventScopedR1CutManifestTests(unittest.TestCase):
    def test_current_cut_is_exact_and_frozen(self):
        ...
        self.assertEqual(result.resolved_count, 306)
        self.assertEqual(result.candidate_count, 306)
        self.assertEqual(result.implementation_count, 154)
        self.assertEqual(result.test_count, 148)
