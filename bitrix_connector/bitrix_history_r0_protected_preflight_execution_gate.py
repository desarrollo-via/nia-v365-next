"""Compuerta separada y fail-closed para un launcher M6 ya compuesto."""
...
from .bitrix_history_r0_protected_preflight_cli import (
...
PROTECTED_PREFLIGHT_REAL_CONFIRMATION = (
    "EJECUTAR PREFLIGHT R0 REAL PROTEGIDO UNA SOLA VEZ"
)
_RESULT_KEYS = frozenset(field.name for field in fields(ProtectedPreflightSnapshot))
...
class ProtectedPreflightExecutionGateSnapshot:
    state: Literal["READY", "NO-GO", "CANCELLED"] = "NO-GO"
    reason: str = "protected_preflight_execution_gate_not_started"
...
def _emit(snapshot: ProtectedPreflightExecutionGateSnapshot) -> None:
...
def _validated_result(
    ...
    *,
    ...
    if not isinstance(payload, Mapping) or frozenset(payload) != _RESULT_KEYS:
        raise TypeError("protected_preflight_execution_result_invalid")
    ...
    if (
        ...
        raise TypeError("protected_preflight_execution_barrier_degraded")
    if state == "READY" and (
        ...
        raise TypeError("protected_preflight_execution_ready_invalid")
    if state == "NO-GO" and payload["failure_category"] == "none":
        raise TypeError("protected_preflight_execution_no_go_category_missing")
    diagnostic_values = (
    ...
    if payload["identity_diagnostic_available"]:
        if (
            ...
            raise TypeError("protected_preflight_execution_identity_diagnostic_invalid")
    elif any(diagnostic_values) or payload["identity_mismatch_count"] != 0:
        raise TypeError("protected_preflight_execution_identity_diagnostic_invalid")

    reason = {
        "READY": "protected_preflight_execution_ready",
        "NO-GO": "protected_preflight_execution_no_go",
        "CANCELLED": "protected_preflight_execution_cancelled",
    }[state]
...
def main(
    ...
    *,
    launcher: PreparedProtectedPreflightLauncher | None = None,
) -> int:
    """Invoca una vez un launcher ya compuesto; nunca lo compone ni reintenta."""

    selected_argv = tuple(sys.argv[1:] if argv is None else argv)
    dotenv_path = _parse_explicit_path(selected_argv)
    if dotenv_path is None:
        snapshot = ProtectedPreflightExecutionGateSnapshot(
            reason="protected_preflight_execution_gate_rejected"
        )
        _emit(snapshot)
        return 2
    if type(launcher) is not PreparedProtectedPreflightLauncher:
        snapshot = ProtectedPreflightExecutionGateSnapshot(
            reason="protected_preflight_execution_launcher_required"
        )
        _emit(snapshot)
        return 2
    ...
    try:
        ...
        snapshot = _validated_result(payload, exit_code=exit_code)
    except KeyboardInterrupt:
        snapshot = ProtectedPreflightExecutionGateSnapshot(
            state="CANCELLED",
            reason="protected_preflight_execution_cancelled",
            launcher_calls=launcher_calls,
            resources_closed=False,
        )
    except BaseException:
        snapshot = ProtectedPreflightExecutionGateSnapshot(
            reason="protected_preflight_execution_failed_safe",
            launcher_calls=launcher_calls,
            resources_closed=False,
        )
    finally:
